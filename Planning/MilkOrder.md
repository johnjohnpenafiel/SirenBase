# Milk Order Tool - Feature Documentation

## Tool Overview

The **Milk Order Tool** is the second tool in SirenBase, designed to streamline the milk inventory counting and ordering process at specialty coffee retail locations. This tool replaces the manual paper logbook system with an efficient, accurate, and user-friendly digital interface.
...
2. Clicks "Milk Order" card → Routes to `/tools/milk-order`
...
2. Opens Milk Order tool → Sees "Continue to Morning Count"
...
#### milk_order_milk_types Table

```sql
CREATE TABLE milk_order_milk_types (
...
CREATE INDEX idx_milk_types_active ON milk_order_milk_types(active);
```
...
#### milk_order_par_levels Table

```sql
CREATE TABLE milk_order_par_levels (
    id VARCHAR(36) PRIMARY KEY,
    milk_type_id VARCHAR(36) NOT NULL UNIQUE REFERENCES milk_order_milk_types(id) ON DELETE CASCADE,
...
#### milk_order_sessions Table

```sql
CREATE TABLE milk_order_sessions (
    id VARCHAR(36) PRIMARY KEY,
...
CREATE INDEX idx_sessions_date ON milk_order_sessions(session_date);
CREATE INDEX idx_sessions_status ON milk_order_sessions(status);
```
...
#### milk_order_entries Table

```sql
CREATE TABLE milk_order_entries (
    id VARCHAR(36) PRIMARY KEY,
    session_id VARCHAR(36) NOT NULL REFERENCES milk_order_sessions(id) ON DELETE CASCADE,
    milk_type_id VARCHAR(36) NOT NULL REFERENCES milk_order_milk_types(id) ON DELETE CASCADE,
...
CREATE INDEX idx_entries_session ON milk_order_entries(session_id);
```
...
- **milk_order_milk_types → milk_order_par_levels** (one-to-one)
- **milk_order_sessions → milk_order_entries** (one-to-many)
- **milk_order_milk_types → milk_order_entries** (one-to-many)
- **users → milk_order_sessions** (one-to-many via night/morning user IDs)
- **users → milk_order_par_levels** (one-to-many via updated_by)

---

### API Endpoints

**Base Path:** `/api/milk-order/*`

#### Admin - Milk Type Endpoints

**GET `/api/milk-order/admin/milk-types`**
...
**PUT `/api/milk-order/admin/milk-types/:id`**
...
**GET `/api/milk-order/admin/par-levels`**
...
**PUT `/api/milk-order/admin/par-levels/:milk_type_id`**
...
**Base Path:** `/tools/milk-order/*`

- **`/tools/milk-order`** - Landing Page
...
- **`/tools/milk-order/night/foh`** - FOH Count Page
  - MilkOrderCard components with +/- counters
  - Grouped by dairy/non-dairy
  - Progress indicator
  - "Save & Continue to BOH" button

- **`/tools/milk-order/night/boh`** - BOH Count Page
  - Same interface as FOH
  - "Back to FOH" and "Save Night Count" buttons
  - On save → redirects to landing

- **`/tools/milk-order/morning`** - Morning Count Page
...
- **`/tools/milk-order/on-order`** - On Order Page
...
- **`/tools/milk-order/summary/[sessionId]`** - Summary Page
...
- **`/tools/milk-order/history`** - History Page
...
- **`/admin/milk-pars`** - Milk Order Pars Management
...
**Location:** `frontend/components/tools/milk-order/`

- **MilkOrderCard** - Counter component
...
- ✅ **Session Endpoints** (`tests/test_milk_order_sessions.py`):
  - Get today's session
  - Start new session
...
- ✅ **Morning Count** (`tests/test_milk_order_morning.py`):
  - BOH count method
...
- ✅ **Summary & History** (`tests/test_milk_order_summary.py`):
...
- Component tests for MilkOrderCard, MorningCountRow
...
- `milk_order_milk_types.active` (filtering active types)
- `milk_order_sessions.session_date` (unique, daily lookup)
- `milk_order_sessions.status` (status filtering)
- `milk_order_entries.session_id` (foreign key join)

### API Response Times

**Targets:**

- GET `/api/milk-order/milk-types`: < 200ms
- GET `/api/milk-order/sessions/today`: < 200ms
- PUT `/api/milk-order/sessions/:id/night-foh`: < 300ms
- GET `/api/milk-order/sessions/:id/summary`: < 300ms
