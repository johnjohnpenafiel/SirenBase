"""
Milk Count CLI commands for development and testing.

Usage:
    flask milk-count reset --all          # Reset all sessions
    flask milk-count reset --today        # Reset today's session only
    flask milk-count reset --to-foh       # Reset to FOH step
    flask milk-count reset --to-boh       # Reset to BOH step
    flask milk-count reset --to-morning   # Reset to Morning step
    flask milk-count reset --to-on-order  # Reset to On Order step
    flask milk-count reset --completed    # Reset with completed session
    flask milk-count reset --with-history # Reset with 7 days of history
...
milk_count_cli = AppGroup('milk-count', help='Milk Count development commands')
...
def delete_all_sessions():
    """Delete all milk count sessions and their entries."""
    MilkCountEntry.query.delete()
    MilkCountSession.query.delete()
    db.session.commit()
...
def delete_today_session():
    ...
    if session:
        MilkCountEntry.query.filter_by(session_id=session.id).delete()
...
def populate_foh_counts(session):
    ...
    entries = MilkCountEntry.query.filter_by(session_id=session.id).all()
...
@milk_count_cli.command('reset')
@click.option('--all', 'reset_all', is_flag=True, help='Delete ALL milk count sessions')
...
def reset_command(reset_all, reset_today, to_foh, to_boh, to_morning, to_on_order, completed, with_history):
    """
    Reset milk count sessions for testing.

    Examples:
        flask milk-count reset --all
        flask milk-count reset --today
        flask milk-count reset --to-morning
        flask milk-count reset --with-history
    """
    check_dev_mode()
    ...
    if reset_all:
        count = MilkCountSession.query.count()
        delete_all_sessions()
