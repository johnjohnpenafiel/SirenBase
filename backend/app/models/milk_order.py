"""
Milk Order models for Tool 2.

This module contains models for the Milk Order System:
- MilkType: Milk type definitions (e.g., Whole, 2%, Oat)
- MilkOrderParLevel: Target inventory levels per milk type
- MilkOrderSession: Daily counting sessions (one per day)
- MilkOrderEntry: Individual milk counts within a session
"""
...
class SessionStatus(str, Enum):
    """Status of a milk order session."""
...
class MilkType(db.Model):
    ...
    __tablename__ = 'milk_order_milk_types'
    ...
    # Relationships
    par_level: Mapped[Optional["MilkOrderParLevel"]] = relationship(
        "MilkOrderParLevel",
        back_populates="milk_type",
        uselist=False,
        cascade="all, delete-orphan"
    )
    entries: Mapped[List["MilkOrderEntry"]] = relationship(
        "MilkOrderEntry",
        back_populates="milk_type",
        cascade="all, delete-orphan"
    )
...
class MilkOrderParLevel(db.Model):
    """
    ...
    __tablename__ = 'milk_order_par_levels'
    ...
    milk_type_id: Mapped[str] = mapped_column(
        String(36),
        ForeignKey('milk_order_milk_types.id', ondelete='CASCADE'),
        nullable=False,
        unique=True
    )
    ...
    def __repr__(self) -> str:
        """String representation for debugging."""
        return f'<MilkOrderParLevel {self.milk_type_id[:8]} - Par: {self.par_value}>'


class MilkOrderSession(db.Model):
    """
    Daily milk order session model.
    ...
    __tablename__ = 'milk_order_sessions'
    ...
    def __repr__(self) -> str:
        """String representation for debugging."""
        return f'<MilkOrderSession {self.session_date} - {self.status}>'


class MilkOrderEntry(db.Model):
    """
    Individual milk order entry within a session.
    ...
    __tablename__ = 'milk_order_entries'
    __table_args__ = (
        UniqueConstraint('session_id', 'milk_type_id', name='uq_milk_order_entries_session_milk_type'),
    )
    ...
    # References
    # Indexed via uq_milk_order_entries_session_milk_type unique constraint
    session_id: Mapped[str] = mapped_column(
        String(36),
        ForeignKey('milk_order_sessions.id', ondelete='CASCADE'),
        nullable=False
    )
    milk_type_id: Mapped[str] = mapped_column(
        String(36),
    ...
    # Relationships
    session: Mapped["MilkOrderSession"] = relationship(
        "MilkOrderSession",
    ...
    def __repr__(self) -> str:
        ...
        return f'<MilkOrderEntry {self.milk_type_id[:8]} - FOH: {foh}, BOH: {boh}>'
