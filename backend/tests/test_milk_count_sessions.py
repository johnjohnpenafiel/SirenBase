"""
Tests for Milk Count session endpoints (workflow management).
...
class TestGetTodaySession:
    """Tests for GET /api/milk-count/sessions/today."""

    def test_get_today_session_none(self, client, staff_headers, app):
        """Test getting today's session when none exists."""
        response = client.get('/api/milk-count/sessions/today', headers=staff_headers)
    ...
    def test_get_today_session_unauthenticated(self, client):
        ...
        response = client.get('/api/milk-count/sessions/today')

        assert response.status_code == 401


class TestStartSession:
    """Tests for POST /api/milk-count/sessions/start."""
    ...
    def test_start_session_success(self, client, staff_headers, app):
        ...
        response = client.post('/api/milk-count/sessions/start', headers=staff_headers)
        ...
        session = MilkCountSession.query.get(data['session']['id'])
    ...
    def test_start_session_creates_entries_for_active_types_only(self, client, staff_headers, app):
        ...
        assert response.status_code == 201
        session = MilkCountSession.query.get(response.json['session']['id'])
        assert len(session.entries) == 1  # Only active type


class TestGetSession:
    """Tests for GET /api/milk-count/sessions/:id."""
    ...
    def test_get_session_success(self, client, staff_headers, app):
        ...
        response = client.get(f'/api/milk-count/sessions/{session.id}', headers=staff_headers)
    ...
    def test_get_session_not_found(self, client, staff_headers):
        ...
        response = client.get('/api/milk-count/sessions/nonexistent-id', headers=staff_headers)

        assert response.status_code == 404


class TestSaveNightFOH:
    """Tests for PUT /api/milk-count/sessions/:id/night-foh."""
    ...
    def test_save_night_foh_success(self, client, staff_headers, app):
        ...
        entry = MilkCountEntry(session_id=session.id, milk_type_id=milk_type.id)
        db.session.add(entry)
        db.session.commit()

        response = client.put(
            f'/api/milk-count/sessions/{session.id}/night-foh',
            json={
                'counts': [
...
class TestSaveNightBOH:
    """Tests for PUT /api/milk-count/sessions/:id/night-boh."""
    ...
    def test_save_night_boh_success(self, client, staff_headers, app):
        ...
        entry = MilkCountEntry(session_id=session.id, milk_type_id=milk_type.id, foh_count=10)
        db.session.add(entry)
        db.session.commit()

        response = client.put(
            f'/api/milk-count/sessions/{session.id}/night-boh',
            json={
                'counts': [
...
class TestSaveMorningCount:
    """Tests for PUT /api/milk-count/sessions/:id/morning."""
    ...
    def test_save_morning_count_boh_method(self, client, staff_headers, app):
        ...
        response = client.put(
            f'/api/milk-count/sessions/{session.id}/morning',
            json={
                'counts': [
                    {
                        'milk_type_id': milk_type.id,
...
class TestGetSessionSummary:
    ...
    def test_get_summary_order_cannot_be_negative(self, client, staff_headers, app):
        ...
        par = MilkCountParLevel(milk_type_id=milk_type.id, par_value=30)  # Low par
        db.session.add(par)
    ...
    def test_get_summary_totals(self, client, staff_headers, app):
        ...
        for mt in types:
            par = MilkCountParLevel(milk_type_id=mt.id, par_value=50)
            db.session.add(par)
    ...
    def test_get_summary_not_found(self, client, staff_headers):
        ...
        response = client.get(
            '/api/milk-count/sessions/nonexistent-id/summary',
            headers=staff_headers
        )
...
class TestGetHistory:
    """Tests for GET /api/milk-count/history."""
    ...
    def test_get_history_success(self, client, staff_headers, app):
        ...
        response = client.get('/api/milk-count/history', headers=staff_headers)
    ...
    def test_get_history_pagination(self, client, staff_headers, app):
        ...
        response = client.get(
            '/api/milk-count/history?limit=2&offset=2',
            headers=staff_headers
        )
    ...
    def test_get_history_filter_by_status(self, client, staff_headers, app):
        ...
        sessions = [
            MilkCountSession(session_date=date.today(), status=SessionStatus.COMPLETED.value),
            MilkCountSession(session_date=date.today() - timedelta(days=1), status=SessionStatus.MORNING.value),
            MilkCountSession(session_date=date.today() - timedelta(days=2), status=SessionStatus.COMPLETED.value)
        ]
        db.session.add_all(sessions)
        db.session.commit()

        response = client.get(
            '/api/milk-count/history?status=completed',
            headers=staff_headers
        )
...
class TestFullWorkflow:
    """Integration tests for complete milk count workflow."""
    ...
    def test_complete_workflow(self, client, staff_headers, app):
        ...
        for i, mt in enumerate(types):
            par = MilkCountParLevel(milk_type_id=mt.id, par_value=50 + i * 10)
            db.session.add(par)
        db.session.commit()
        ...
        response = client.put(
            f'/api/milk-count/sessions/{session_id}/night-foh',
            json={
                'counts': [
        ...
        response = client.put(
            f'/api/milk-count/sessions/{session_id}/night-boh',
            json={
                'counts': [
        ...
        response = client.put(
            f'/api/milk-count/sessions/{session_id}/morning',
            json={
                'counts': [
                    {
                        'milk_type_id': types[0].id,
