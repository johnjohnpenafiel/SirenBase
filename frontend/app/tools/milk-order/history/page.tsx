/**
 * Milk Order - History Page
 *
 * Shows past milk order sessions with status and links to summaries.
 *
...
import { MilkOrderHistorySkeleton } from "@/components/tools/milk-order/MilkOrderHistorySkeleton";
...
const STATUS_DISPLAY: Record<MilkOrderSessionStatus, {
  label: string;
  color: string;
...
export default function HistoryPage() {
  ...
  const [sessions, setSessions] = useState<MilkOrderSession[]>([]);
  ...
  const isMissed = (session: MilkOrderSession) => {
  ...
  const loadSessions = async (loadMore = false) => {
    try {
      ...
      const response = await apiClient.getMilkOrderHistory({
        limit,
  ...
  const handleSessionClick = (session: MilkOrderSession) => {
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center animate-fade-in">
                  ...
                  <p className="text-sm text-muted-foreground mb-6 max-w-xs">
                    Complete your first milk order to see it here
                  </p>
                  <Button
                    onClick={() => router.push("/tools/milk-order")}
