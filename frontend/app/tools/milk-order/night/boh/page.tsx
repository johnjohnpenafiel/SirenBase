/**
 * Milk Order - Night BOH (Back of House) Count Page
 *
...
export default function NightBOHPage() {
  ...
  useEffect(() => {
    router.prefetch("/tools/milk-order");
  }, [router]);
  ...
  const handleSaveNightCount = async () => {
    ...
    try {
      ...
      await apiClient.saveMilkOrderNightBOH(session.id, { counts: bohCounts });
      router.push("/tools/milk-order");
    } catch (error: unknown) {
          ...
          <div className="sticky top-[72px] z-10 px-4 md:px-8">
            ...
            >
              ...
              <div className="mb-3">
                <BackButton
                  variant="icon-only"
                  href="/tools/milk-order/night/foh"
                  label="Back to FOH"
                />
              </div>
