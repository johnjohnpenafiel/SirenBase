/**
 * Milk Order - Night FOH (Front of House) Count Page
 *
...
import { MilkOrderCard } from "@/components/tools/milk-order/MilkOrderCard";
import { ArrowRight } from "lucide-react";
import { MilkOrderStepSkeleton } from "@/components/tools/milk-order/MilkOrderStepSkeleton";
import apiClient from "@/lib/api";
import { toast } from "sonner";
import { cn, getErrorMessage } from "@/lib/utils";
import type { MilkType, MilkOrderSession, MilkOrderState } from "@/types";
...
export default function NightFOHPage() {
  ...
  const [counts, setCounts] = useState<MilkOrderState>({});
  ...
  useEffect(() => {
    router.prefetch("/tools/milk-order/night/boh");
  }, [router]);
  ...
  const loadData = async () => {
    try {
      ...
      if (currentSession.status === "morning") {
        ...
        router.replace("/tools/milk-order/morning");
        return;
      }
      ...
      if (currentSession.id) {
        try {
          ...
          const initialCounts: MilkOrderState = {};
  ...
  const handleSaveAndContinue = async () => {
    ...
    try {
      ...
      await apiClient.saveMilkOrderNightFOH(session.id, { counts: fohCounts });
      router.push("/tools/milk-order/night/boh");
    } catch (error: unknown) {
      toast.error(getErrorMessage(error, "Failed to save FOH count"));
    } finally {
      setSaving(false);
    }
  };
