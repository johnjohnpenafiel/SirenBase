...
import { OnOrderRow } from "@/components/tools/milk-order/OnOrderRow";
...
export default function OnOrderPage() {
  ...
  const [onOrders, setOnOrders] = useState<MilkOrderState>({});
  ...
  useEffect(() => {
    if (session?.id) {
      router.prefetch(`/tools/milk-order/summary/${session.id}`);
    }
  }, [session?.id, router]);
  ...
  const loadData = async () => {
    try {
      ...
      if (currentSession.id) {
        ...
        const initialOnOrders: MilkOrderState = {};
  ...
  const handleSaveOnOrder = async () => {
    ...
    try {
      ...
      await apiClient.saveMilkOrderOnOrder(session.id, { on_orders: onOrderData });
