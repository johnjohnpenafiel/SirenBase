/**
 * Milk Order Tool - Landing Page
 *
 * Entry point for the milk order workflow.
...
import { MilkOrderLandingSkeleton } from "@/components/tools/milk-order/MilkOrderLandingSkeleton";
import apiClient from "@/lib/api";
import { toast } from "sonner";
import { cn, parseLocalDate, getErrorMessage } from "@/lib/utils";
import type { MilkOrderSession, MilkOrderSessionStatus } from "@/types";

// Status configuration for display
const STATUS_CONFIG: Record<MilkOrderSessionStatus | "none", {
  title: string;
  stepNumber: string;
  status: string;
  statusColor: string;
  ...
  none: {
    ...
    route: "/tools/milk-order/night/foh",
  },
  night_foh: {
    title: "Night FOH",
    stepNumber: "01",
  ...
  night_boh: {
    ...
    route: "/tools/milk-order/night/boh",
  },
  morning: {
    title: "Morning BOH",
    stepNumber: "03",
    ...
    route: "/tools/milk-order/morning",
  },
  on_order: {
    title: "On Order",
    stepNumber: "04",
    ...
    route: "/tools/milk-order/on-order",
  },
  completed: {
    title: "Milk Order",
    stepNumber: "OK",
    ...
    description: "Today's milk order is complete. View the summary below.",
...
export default function MilkOrderPage() {
  ...
  const loadSession = async () => {
    try {
      const response = await apiClient.getMilkOrderTodaySession();
  ...
  const handleAction = async () => {
    ...
    if (status === "completed" && session) {
      router.push(`/tools/milk-order/summary/${session.id}`);
      return;
    }
    ...
    if (status === "none") {
      setStarting(true);
      try {
        const response = await apiClient.startMilkOrderSession();
            ...
            >
              ...
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  onClick={() => router.push("/tools/milk-order/history")}
                >
