...
import { MilkOrderSummarySkeleton } from "@/components/tools/milk-order/MilkOrderSummarySkeleton";
import {
  Collapsible,
  CollapsibleContent,
  CollapsibleTrigger,
...
import type { MilkOrderSummaryEntry, MilkOrderSession, MilkCategory } from "@/types";
...
function SummaryCard({ entry }: { entry: MilkOrderSummaryEntry }) {
...
function MobileSummarySection({ title, entries }: { title: string; entries: MilkOrderSummaryEntry[] }) {
...
// Desktop table row
function SummaryTableRow({ entry }: { entry: MilkOrderSummaryEntry }) {
  return (
...
export default function SummaryPage() {
  ...
  const [summary, setSummary] = useState<MilkOrderSummaryEntry[]>([]);
  ...
  useEffect(() => {
    router.prefetch("/tools/milk-order/history");
  }, [router]);

  const loadSummary = async () => {
    try {
      const response = await apiClient.getMilkOrderSummary(sessionId);
